...
inline double median(string expr, py::kwargs arrays) {
    using T = double;
    ...
    // Evaluate expression for each element and store results
...
#pragma omp simd
    ...
    if (n % 2 == 1) {
        ...
        nth_element(values.begin(), mid, values.end());
        return *mid;
    } else {
        // Even: average of two middle elements
        auto mid2 = values.begin() + n / 2;
        nth_element(values.begin(), mid2, values.end());
        double upper = *mid2;

        // The max of the lower half is the other middle element
        auto mid1 = max_element(values.begin(), mid2);
        return (*mid1 + upper) / 2.0;
    }
}